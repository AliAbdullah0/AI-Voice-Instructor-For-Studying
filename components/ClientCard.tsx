...
const ClientCard = ({id,name,topics,createdAt,description,userId,membersLength,className}:CourseType) => {
  ...
  return (
    ...
    <div className='flex flex-col gap-1 w-full'>
        ...
        <div>
            <ShinyText className='text-sm line-clamp-2 mt-2' text={description || `Take this course and learn ${formatTopics}`} speed={3} disabled={false} />
        </div>
...
const Badge = ({topic,className}:{topic:string,className?:string})=>{
    return(
        <div className={`flex items-center line-clamp-1 hover:bg-white/20 hover:transition-all rounded-xl text-xs bg-white/10 px-2 py-1.5 ${className}`}>
            <p className="line-clamp-1 first-letter:uppercase">{topic}</p>
        </div>
    )
}
